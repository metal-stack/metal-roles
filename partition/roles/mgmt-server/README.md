...
| mgmt_server_vrfs                      | no        | Additional BGP instances in VRFs. Each entry accepts: name, optional router_id, optional bgp_timers, optional routemap_out, and the lists unnumbered_interfaces and announcements. |
...
| mgmt_server_masquerade_exclude_destinations | no  | Entries exempted from masquerading, inserted as RETURN above the MASQUERADE rule. Either a destination prefix, or a mapping with destination and an optional source to restrict the exemption to traffic originating from that prefix. |
