...
| gardener_logging_ingress_dns                      |           | `loki.{{ metal_control_plane_ingress_dns }}` | DNS for the loki ingress (used in `gardener_logging_alloy_loki_write_endpoints` examples)                                                                                      |
| gardener_logging_ingress_loki_basic_auth_user     |           | `promtail`                                   | Basic auth user for the external loki ingress                                                                                                                                  |
| gardener_logging_ingress_loki_basic_auth_password | yes       |                                              | Basic auth password for the external loki ingress                                                                                                                              |
...
For WAL settings, `logging_alloy_prometheus_write_endpoints`, and other Alloy options, set the `logging_alloy_*` variables from [logging-common](../logging-common/) directly in your inventory — they are used by the shared deployment task.
