...
| ------------------------------------------------- | --------- | ------------------------------------------------------------- |
| gardener_logging_alloy_chart_version              |           | Helm chart version for alloy — deploy Alloy when set          |
| gardener_logging_alloy_chart_repo                 |           | Repository for alloy                                          |
| gardener_logging_promtail_chart_version           |           | Helm chart version for promtail — deploy Promtail when set    |
| gardener_logging_promtail_chart_repo              |           | Repository for promtail                                       |
...
| Name                                                     | Mandatory | Description                                                                                                                                                                                                                                                                                                     |
| -------------------------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| gardener_logging_alloy_chart_version                     |           | Helm chart version for alloy. Alloy is deployed only when this is set.                                                                                                                                                                                                                                          |
| gardener_logging_alloy_chart_repo                        |           | Repository for alloy                                                                                                                                                                                                                                                                                            |
...
1. If you are pushing Alloy self-metrics to Thanos Receive, migrate the credentials first — the monitoring role will fail immediately on the next deploy if the old variable is still set. See [Thanos Receive credentials](#thanos-receive-credentials) below.
2. Add `gardener_logging_alloy_chart_version` and `gardener_logging_alloy_chart_repo` to your config alongside the existing Promtail vars. Both will run in parallel — Loki will receive duplicate log entries during this window.
3. Before removing Promtail, verify:
