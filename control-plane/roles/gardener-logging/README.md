...
| **Cutover complete**              | `true`                           | `false`                             | Set `gardener_logging_promtail_migrate_cleanup: true`, `event_exporter_enabled: false`, and `event_exporter_migrate_cleanup: true` (in monitoring config) and re-run. Remove the cleanup variables afterwards. |
