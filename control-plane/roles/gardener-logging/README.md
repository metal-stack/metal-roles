...
- **One new label.** `pod_uid` is added to pod log streams. All other labels (`app`, `instance`, `component`, `job`, `namespace`, etc.) are identical to Promtail's chart defaults.
...
5. Set `gardener_logging_promtail_enabled: false` and re-run. Remove Promtail from each seed: `helm uninstall promtail -n {{ gardener_logging_namespace }}`.
6. Remove `event_exporter_name`/`event_exporter_tag` from your monitoring config — only needed for Promtail's event pipeline. See the [monitoring role migration guide](../monitoring/README.md#event-exporter-is-now-opt-in).
